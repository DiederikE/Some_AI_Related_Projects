...
def get_generalization_loss(args, data_train, data_transfer):
    ...
    print('Loss transfer:', loss_transfer)
...
def main(args):
    ...
    # cg.draw_pydot_graph(labels=['Long', 'Temp'])
    ...
    for i in range(number_vars):
        for j in range(i):
            ...
            if ((left_right_value == -1 and right_left_value == -1)
                ...
                random.shuffle(data)
                data.sort(key=lambda x: x[0][0])  # Transfer distribution determined on Longitude
    ...
    graph.write_png('img_scriptie/exp1_gen_two_var.png')
