...
def main(args):
    ...
    random.shuffle(data)
    data.sort(key=lambda x: x[0][0]) # TODO: Determines distributions
    pc_data = np.asarray(data)
    pc_data = np.squeeze(pc_data, axis=2)

    pc_data_rest = [[x[1], x[2], x[3], x[4]] for x in pc_data]
    pc_data_first_element = [[x[0]] for x in pc_data]

    # default parameter
    cg = cdnod(pc_data_rest, np.asarray(pc_data_first_element))

    # visualization using pydot
    # note that the last node is the c_indx
    # cg.draw_pydot_graph(labels=['Temperatuur', 'Neerslag', 'Lengtegraad', 'Zonneschijn', 'Hoogte'])

    pyd = GraphUtils.to_pydot(cg.G, labels=['Temperatuur', 'Neerslag', 'Lengtegraad', 'Zonneschijn', 'Hoogte'])
    pyd.write_png('img_scriptie/exp5_cdnod_multi.png')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--file_name', type=str,
                        default='data/alt_temp_prec_long_sun.txt',
                        help='input file')
