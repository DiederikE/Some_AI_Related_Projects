...
for r in range(repeats):
    print(r, '/', repeats)
    directions = []
    # Generate random causal graph between m variables --
    for i in range(m):
        for j in range(i):
    ...
    for j in range(num_runs):
        ...
        for i in range(num_training):
            ...
            # Generate P(A) and P(B|A) from random dirichlet distribution --
            pi_A_1 = np.random.dirichlet(np.ones(N))
            pi_B_A = np.random.dirichlet(np.ones(N), size=N)

            # Set this as the truth --
            model.set_ground_truth(pi_A_1, pi_B_A)

            # Generate training data based on given distributions --
            x_train_original = torch.from_numpy(generate_data_categorical(
                train_batch_size, pi_A_1, pi_B_A))

            # calculate losses on training data
            with torch.no_grad():
            ...
            for k in transfers:
                ...
                # Generate P(A) in transfer distribution --
                ...
                for _ in range(num_gradient_steps):
                    ...
                    # Generate transfer data based on new transfer P(A) and same P(B|A)
                    x_train = torch.from_numpy(generate_data_categorical(transfer_batch_size, pi_A_2, pi_B_A))
                    # calculate the losses of the 2 models in the transfer distribution
                    # and cumulate them with the
                    with torch.no_grad():
